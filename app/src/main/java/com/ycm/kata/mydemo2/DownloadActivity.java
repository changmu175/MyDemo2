...
import com.ycm.kata.networkmodule.callback.DownloadProgressCallBack;
...
public class DownloadActivity extends AppCompatActivity {
    ...
    public void onDownloadFile1(View view) {//下载回调是在异步里处理的
        String url = "http://m1.auto.itc.cn/live/sohulive_officialOnlineRelease_1.0.5_11140938.apk";
        String name = getApkNameFromUrl(url);
        String rootPath = Environment.getExternalStorageDirectory().getPath() + File.separator + "A1";
        File rootFile = new File(rootPath);
        if (!rootFile.exists() && !rootFile.mkdirs()) {
            return;
        }

        File apkFile = new File(rootFile, name);
        try {
        ...
        NetworkManager.downLoad("http://m1.auto.itc.cn/live/sohulive_officialOnlineRelease_1.0.5_11140938.apk")
                .savePath(rootPath)//默认在：/storage/emulated/0/Android/data/包名/files/1494647767055
                .saveName(name)//默认名字是时间戳生成的
