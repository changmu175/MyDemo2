...
import com.ycm.kata.networkmodule.callback.CallBack;
import com.ycm.kata.networkmodule.callback.CallBackProxy;
import com.ycm.kata.networkmodule.callback.CallClazzProxy;
import com.ycm.kata.networkmodule.func.ApiResultFunc;
import com.ycm.kata.networkmodule.func.CacheResultFunc;
import com.ycm.kata.networkmodule.func.RetryExceptionFunc;
import com.ycm.kata.networkmodule.model.ApiResult;
import com.ycm.kata.networkmodule.model.CacheResult;
import com.ycm.kata.networkmodule.subscriber.CallBackSubsciber;
import com.ycm.kata.networkmodule.utils.RxUtil;
...
//                .retryWhen(new RetryExceptionFunc(retryCount, retryDelay, retryIncreaseDelay))
    ...
    public <T> Disposable execute(CallBack<T> callBack) {
        return execute1(new CallBackProxy<ApiResult<T>, T>(callBack) {
        });
    }
    ...
    public <T> Disposable execute1(CallBackProxy<? extends ApiResult<T>, T> proxy) {
        build();
        Observable<ApiResult<T>> observable = generateRequest()
    ...
    public <T> Disposable execute2(CallBackProxy<? extends ApiResult<T>, T> proxy) {
        build();
        Observable<ApiResult<T>> observable = generateRequest()
                .map(new ApiResultFunc(proxy != null ? proxy.getType() : new TypeToken<ResponseBody>() {
                }.getType()))
