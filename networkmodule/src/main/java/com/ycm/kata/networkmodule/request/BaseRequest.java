...
package com.ycm.kata.networkmodule.request;

import android.content.Context;
import android.text.TextUtils;

import com.ycm.kata.networkmodule.NetworkManager;
import com.ycm.kata.networkmodule.api.ApiService;
import com.ycm.kata.networkmodule.interceptor.BaseDynamicInterceptor;
import com.ycm.kata.networkmodule.interceptor.HeadersInterceptor;
...
//    protected HttpsUtils.SSLParams sslParams;
    ...
    public BaseRequest(String url) {
        this.url = url;
        context = NetworkManager.getContext();
        NetworkManager config = NetworkManager.getInstance();
...
//        if (cacheTime <= -1) cacheTime = NetworkManager.DEFAULT_CACHE_NEVER_EXPIRE;
...
//            OkHttpClient.Builder builder = NetworkManager.getOkHttpClientBuilder();
...
//            return builder;
//        } else {
//            final OkHttpClient.Builder newClientBuilder = NetworkManager.getOkHttpClient().newBuilder();
//            if (readTimeOut > 0)
...
//            if (cookies.size() > 0) NetworkManager.getCookieJar().addCookies(cookies);
...
//        builder.addNetworkInterceptor(new StethoInterceptor());
    ...
    private Retrofit.Builder generateRetrofit() {
        if (converterFactories.isEmpty() && adapterFactories.isEmpty()) {
            return NetworkManager.getRetrofitBuilder().baseUrl(baseUrl);
        } else {
            ...
            if (!converterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                List<Converter.Factory> listConverterFactory = NetworkManager.getRetrofit().converterFactories();
            ...
            if (!adapterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                List<CallAdapter.Factory> listAdapterFactory = NetworkManager.getRetrofit().callAdapterFactories();
...
//        final RxCache.Builder rxCacheBuilder = NetworkManager.getRxCacheBuilder();
//        switch (cacheMode) {
//            case NO_CACHE://不使用缓存
...
//                if (this.cache == null) {
//                    File cacheDirectory = NetworkManager.getCacheDirectory();
//                    if (cacheDirectory == null) {
//                        cacheDirectory = new File(NetworkManager.getContext().getCacheDir(), "okhttp-cache");
//                    } else {
//                        if (cacheDirectory.isDirectory() && !cacheDirectory.exists()) {
//                            cacheDirectory.mkdirs();
//                        }
//                    }
//                    this.cache = new Cache(cacheDirectory, Math.max(5 * 1024 * 1024, NetworkManager.getCacheMaxSize()));
//                }
//                String cacheControlValue = String.format("max-age=%d", Math.max(-1, cacheTime));
//                final CacheInterceptor REWRITE_CACHE_CONTROL_INTERCEPTOR = new CacheInterceptor(NetworkManager.getContext(), cacheControlValue);
//                final CacheInterceptorOffline REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE = new CacheInterceptorOffline(NetworkManager.getContext(), cacheControlValue);
